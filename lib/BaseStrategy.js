...
class BaseStrategy extends passport.Strategy {
  constructor(options, verify, newProviderOnConstruct = true) {
    super();

    if (typeof options == 'function') {
      throw new Error('Mandatory OpenID options missing');
    }

    if (!verify || typeof verify !== 'function') {
      throw new Error('Mandatory verify callback missing');
    }
    ...
    if (!options.getOpenIdConfig) {
      ...
      this._stateData = options.stateData || {};
      this._tokenValidationCertificate = options.tokenValidationCertificate;
  ...
  updateOptions(options = {}) {
    ...
    if (options.tokenValidationCertificate) {
      this._tokenValidationCertificate = options.tokenValidationCertificate;
    }
    ...
    this._skipUserProfile = (options.skipUserProfile === undefined) ? function() {
      if ((options.passReqToCallback && this._verify.length >= 10) || (!options.passReqToCallback && this._verify.length >= 9)) { 
        return false; 
      }
  ...
  authenticate(req, options = {}) {
    if (this._oauth2 === null) {
      throw new Error(`Can't get authenticate without a provider defined`);
    }
    const self = this;
    ...
    var meta = {
      ...
      grantType: this._grantType || 'authorization_code',
    };
    ...
    if (req.query && req.query.code) {
      function restored(err, ctx, state) {
        ...
        var params = { grant_type: meta.grantType };
        ...
        self._oauth2.getOAuthAccessToken(code, params, function(err, accessToken, refreshToken, params) {
          ...
          if (self._tokenValidationCertificate) {
            try {
              jwtlib.verify(idToken, self._tokenValidationCertificate) // just to check token signature
            } catch (err) {
          ...
          if (typeof claims.aud === 'string') {
            if (claims.aud !== self._oauth2._clientId && claims.aud.indexOf(self._oauth2._clientId) === -1) { return self.fail({ message: 'ID token not intended for this relying party.' }, 403); }
          } else {
      ...
      if (scope) {
        if (Array.isArray(scope)) {
          const filteredScopes = scope.filter(scopeItem => scopeItem !== 'openid')
  ...
  _shouldLoadUserProfile(req, claims, done) {
