...
class MultiStrategy extends Strategy {
  ...
  authenticate(req, options) {
    this._options.getOpenIdConfig(req, (err, openIdOptions) => {
      ...
      const stateStoreKey = openIdOptions.stateStoreKey || `${this.name}:${url.parse(openIdOptions.authorizationURL).hostname}`;
      const stateStore = new SessionStateStore({ key: stateStoreKey, isCustomStateKey: !!openIdOptions.stateStoreKey, skipStateValidation: openIdOptions.skipStateValidation });
